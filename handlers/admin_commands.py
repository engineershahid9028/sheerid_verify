    ...
    try:
        ...
        if db.add_balance(target_user_id, amount):
            ...
            await update.message.reply_text(
                ...
                f"Current Balance: {user['balance']} pts"
            )
        else:
...
async def genkey_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    ...
    if not context.args or len(context.args) < 2:
        await update.message.reply_text(
            ...
            "Examples:\n"
            "/genkey vip20 20 - Generate a 20-point code (single use, never expires)\n"
            "/genkey vip100 50 10 - Generate a 50-point code (10 uses, never expires)\n"
            "/genkey temp 30 1 7 - Generate a 30-point code (single use, expires in 7 days)"
        )
        return
    ...
    try:
        ...
        if max_uses <= 0:
            await update.message.reply_text("Usage limit must be greater than 0.")
            return
        ...
        if db.create_card_key(key_code, balance, user_id, max_uses, expire_days):
            ...
            await update.message.reply_text("Key already exists or generation failed. Try a different code.")
    except ValueError:
        await update.message.reply_text("Invalid parameter format, please enter valid numbers.")
